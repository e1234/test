...
.factory('GridRenderContainer', ['gridUtil', 'uiGridConstants', function(gridUtil, uiGridConstants) {
  ...
  GridRenderContainer.prototype.getViewPortStyle = function () {
    ...
    if (self.name === 'body') {
      styles['overflow-x'] = self.grid.options.enableHorizontalScrollbar === uiGridConstants.scrollbars.NEVER ? 'hidden' : 'overlay';
      if (!self.grid.isRTL()) {
        if (self.grid.hasRightContainerColumns()) {
          styles['overflow-y'] = 'hidden';
        }
        else {
          styles['overflow-y'] = self.grid.options.enableVerticalScrollbar === uiGridConstants.scrollbars.NEVER ? 'hidden' : 'overlay';
        }
      }
      else {
      ...
      styles['overflow-y'] = self.grid.isRTL() ? (self.grid.options.enableVerticalScrollbar === uiGridConstants.scrollbars.NEVER ? 'hidden' : 'overlay') : 'hidden';
    }
    else {
      styles['overflow-x'] = 'hidden';
      styles['overflow-y'] = !self.grid.isRTL() ? (self.grid.options.enableVerticalScrollbar === uiGridConstants.scrollbars.NEVER ? 'hidden' : 'overlay') : 'hidden';
...
module.service('gridUtil', ['$log', '$window', '$document', '$http', '$templateCache', '$timeout', '$injector', '$q', '$interpolate', 'uiGridConstants',
  ...
  s.rtlScrollType = function rtlScrollType() {
    ...
    var definer = angular.element('<div dir="rtl" style="font-size: 14px; width: 1px; height: 1px; position: absolute; top: -1000px; overflow: overlay">A</div>')[0],
        type = 'reverse';
